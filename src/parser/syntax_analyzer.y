...
%token <node> LSBRA RSBRA
%token <node> LMBRA RMBRA
%token <node> LLBRA RLBRA
%token <node> IF ELSE
%token <node> WHILE RETURN
%token <node> LTHAN
%token <node> MTHAN
%token <node> EQUAL
%token <node> LTHANE
...
var-declaration: type-specifier ID SEMI {$$ = node( "var-declaration", 3, $1,$2,$3);  }
                | type-specifier ID LMBRA INTEGER RMBRA SEMI {$$ = node( "var-declaration", 6, $1,$2,$3,$4,$5,$6);  }
                ;
type-specifier: INT {$$ = node( "type-specifier", 1, $1);  }
                | FLOAT {$$ = node( "type-specifier", 1, $1);  }
                | VOID {$$ = node( "type-specifier", 1, $1);  }
                ;
fun-declaration: type-specifier ID LSBRA params RSBRA compound-stmt {$$ = node( "fun-declaration", 6, $1,$2,$3,$4,$5,$6);  }
                ;
...
param: type-specifier ID {$$ = node( "param", 2, $1,$2);  }
        | type-specifier ID LMBRA RMBRA {$$ = node( "param", 4, $1,$2,$3,$4);  }
        ;
compound-stmt: LLBRA local-declarations statement-list RLBRA {$$ = node( "compound-stmt", 4, $1,$2,$3,$4);  }
            ;
...
expression-stmt: expression SEMI {$$ = node( "expression-stmt", 2,$1,$2);  }
                ...
                ;
selection-stmt: IF LSBRA expression RSBRA statement {$$ = node( "selection-stmt", 5, $1,$2,$3,$4,$5);  }
                | IF LSBRA expression RSBRA statement ELSE statement {$$ = node( "selection-stmt", 7, $1,$2,$3,$4,$5,$6,$7);  }
                ;
iteration-stmt: WHILE LSBRA expression RSBRA statement {$$ = node( "iteration-stmt", 5, $1,$2,$3,$4,$5);  }
                ;
...
expression: var EQUAL expression {$$ = node( "expression", 3,$1,$2,$3);  }
            ...
            ;
var: ID {$$ = node( "var", 1,$1);  }
    | ID LMBRA expression RMBRA {$$ = node( "var", 4,$1,$2,$3,$4);  }
    ;
simple-expression: additive-expression relop additive-expression {$$ = node( "simple-expression", 3,$1,$2,$3);  }
                    ...
                    ;
relop: LTHANE {$$ = node( "relop", 1,$1);  }
        | LTHAN {$$ = node( "relop", 1,$1);  }
        | MTHAN {$$ = node( "relop", 1,$1);  }
        | MTHANE {$$ = node( "relop", 1,$1);  }
        | EQUALE {$$ = node( "relop", 1,$1);  }
...
mulop: MUL {$$ = node( "mulop", 1,$1);  }
        | DIV {$$ = node( "mulop", 1,$1);  }
        ;
factor: LSBRA expression RSBRA {$$ = node( "factor", 3,$1,$2,$3);  }
        | var {$$ = node( "factor", 1,$1);  }
...
float: FLOATPOINT {$$ = node( "float", 1,$1);  }
        ;
call: ID LSBRA args RSBRA {$$ = node( "call", 4,$1,$2,$3,$4);  }
    ;
